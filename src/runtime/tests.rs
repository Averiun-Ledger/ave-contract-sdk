//! Unit tests for the wasmtime runtime (limits, engine, host memory).
//!
//! The file itself is only compiled under `cfg(all(test, feature = "runtime"))`
//! (see the parent module), so no extra gating is needed here.
...
#[test]
fn invalid_module_kind_display_wrong_module_and_signature() {
    let err = RuntimeError::InvalidModule(InvalidModuleKind::UnexpectedImportModule {
        module: "evil".to_string(),
        name: "alloc".to_string(),
    });
    assert_eq!(
        err.to_string(),
        "invalid module: module imports 'alloc' from 'evil', expected module 'env'"
    );

    let err = RuntimeError::InvalidModule(InvalidModuleKind::InvalidImportSignature {
        name: "alloc".to_string(),
        expected: "(param i32) (result i32)".to_string(),
    });
    assert_eq!(
        err.to_string(),
        "invalid module: module imports 'alloc' with an invalid signature, expected (param i32) (result i32)"
    );

    let err = RuntimeError::InvalidModule(InvalidModuleKind::UnknownImportFunction {
        name: "hack".to_string(),
    });
    assert!(err.to_string().contains("'hack'"));

    let err = RuntimeError::InvalidModule(InvalidModuleKind::NonFunctionImport {
        import_type: "memory".to_string(),
    });
...
fn checked_guest_len_accepts_zero_and_normal_values() {
    ...
    assert_eq!(checked_guest_len(1024, 1024).unwrap(), 1024);
}

#[test]
fn checked_guest_len_rejects_negative() {
    use crate::runtime::host::checked_guest_len;
    let err = checked_guest_len(-1, 1024).unwrap_err();
    assert!(matches!(err, ContractError::InvalidLength { len: -1 }));
    assert_eq!(err.to_string(), "invalid length: -1");
}

#[test]
fn checked_guest_len_rejects_above_cap() {
    use crate::runtime::host::checked_guest_len;
    let err = checked_guest_len(2048, 1024).unwrap_err();
    assert!(matches!(
        err,
...
#[test]
fn memory_manager_read_data_at_boundary() {
    // Writing exactly the allocated size and reading back stays in bounds.
    let mut manager = MemoryManager::default();
    let ptr = manager.alloc(8).unwrap();
    manager.write_bytes(ptr, &[9u8; 8]).unwrap();
    assert_eq!(manager.read_data(ptr).unwrap(), &[9u8; 8]);
    // Unknown pointers near usize::MAX must error, never wrap.
    let err = manager.read_data(usize::MAX).unwrap_err();
    assert!(matches!(err, ContractError::InvalidPointer { .. }));
}

#[test]
fn contract_metrics_observe_and_register() {
    use std::time::Duration;

    use crate::runtime::metrics::ContractMetrics;

    let metrics = ContractMetrics::default();
    metrics.observe_contract_execution("success", Duration::from_millis(1));
    metrics.observe_contract_fuel_consumed("success", 1_000);
    metrics.observe_contract_memory_peak("success", 4_096);
