...
fn init_logic(_state: &State, contract_result: &mut sdk::ContractInitCheck) {
...
fn contract_logic(
    ...
    match &context.event {
        ...
        StateEvent::ModThree { data } => {
            if *data == 50 {
                contract_result
                    .reject("Can not change three value, 50 is a invalid value");
                return;
            }
...
fn contract_test_fail() {
    ...
    assert_eq!(
        result.error,
        "Can not change three value, 50 is a invalid value"
    );
    assert!(!result.success);
}
